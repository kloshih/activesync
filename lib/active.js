  ...
  _configure() {
    ...
    for (let key in props) {
      ...
      if (subconfig == null || !isSubclass(type, Active)) {
        const defType = _typeof(def), subType = _typeof(subconfig);
      ...
      switch (req) {
        ...
        case '*': case '+':
          ...
          for (const subkey in subconfig) {
            ...
            const subconf = type.coerceConfig(subconfig[subkey]);
            subconf.$key = subkey;
...
//    return this[kAllProps]
  ...
  static coerceConfig(config) {
    ...
    for (let key in props) {
      let prop = props[key];
      coerced[key] = prop.implicit && implicit ? implicit 
...
//   _events: { value:undefined, configurable:true, writable:true },
//   _eventCount: { value:undefined, configurable:true, writable:true },
// })
// Object.defineProperties(Active.prototype, {
//   _meta: { value:undefined, configurable:true, writable:true },
// })
